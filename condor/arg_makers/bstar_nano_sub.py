...
parser.add_option('-n', '--name', metavar='FILE', type='string', action='store',
                default   =   '',
                dest      =   'name',
                help      =   'A custom name for this argument list (bstar_nano_<name>_args.txt)')
...
outfile = open('../args/bstar_nano'+name_string+'_args.txt','w')
...
for year in ['16','17','18']: 
    if year not in ignore:
        ...
        for file in glob.glob('../../bstarTrees/NanoAOD'+year+'_lists/*_loc.txt'):
