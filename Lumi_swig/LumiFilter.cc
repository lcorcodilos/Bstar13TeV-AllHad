...
LumiFilter::LumiFilter(int year){
    if (year == 17 or year == 2017) {
        boost::property_tree::read_json(std::string("lumi_jsons/golden_jsons/Cert_294927-306462_13TeV_EOY2017ReReco_Collisions17_JSON.txt"), ptree);
    } else if (year == 18 or year == 2018) {
        boost::property_tree::read_json(std::string("lumi_jsons/golden_jsons/Cert_314472-325175_13TeV_17SeptEarlyReReco2018ABC_PromptEraD_Collisions18_JSON.txt"), ptree);
    } else {
...
// extern "C" { 
//     LumiFilter* LumiFilter_new(int year){ return new LumiFilter(year); } 
//     void LumiFilter_eval(LumiFilter* lf, unsigned int run, unsigned int lumi){ lf->eval(run,lumi); } 
// }
